	...
	class pdo_database{
		...
		public function getDetailedInfo($id){
			...
			$info = $stmt->fetch(PDO::FETCH_ASSOC);
			 ...
			 // Check if $info is a valid array before using extract
			...
			echo '<tr style="border-bottom:1px solid #000;"><td style="padding:10px 8px;"><b>Crme record:</b></td><td>Good</td></tr>';
				...
				echo "No detailed information found for ID: $id";
			}
		}
		...
		public function getDetailedInfoPerson($name){
			$sql = "SELECT Address,image,description,phoneNumber FROM missing_persons WHERE fullName = ?";
			
			$stmt = $this->db_conn->prepare($sql);
			$results = $stmt->execute([$name]); // Pass the value directly without using array("$name")
		...
		public function getDetailedInfoVehicleFound($id){
			...
			$stmt = $this->db_conn->prepare($sql);
			$results = $stmt->execute([$id]); // Pass the value directly without using array("$id")
